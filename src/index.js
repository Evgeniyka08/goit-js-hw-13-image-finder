...
import getApiRequest from './js/apiServise.js';
import templateListImages from './templates/list-images-templaye.hbs';
...
function handlerNexPagesOfImages(event) {
  ...
  getApiRequest(keyWord, numberOfPage, userKey).then(data => {
    refs.listOfImages.insertAdjacentHTML('beforeend', templateListImages(data));
    window.scrollTo({
      top: pageHeight,
      behavior: 'smooth',
    });
